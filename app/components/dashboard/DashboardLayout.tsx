...
const Sidebar = ({ isOpen, setIsOpen }: { isOpen: boolean; setIsOpen: (v: boolean) => void }) => {
    ...
    return (
        <>
            ...
            <motion.aside
                className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-black border-r-4 border-black text-white flex flex-col transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
                    }`}
            >
                {/* Brand */}
                <div className="p-8 border-b border-gray-800 flex items-center gap-3">
                    <motion.div
                        ...
                        className="w-10 h-10 bg-[#EAB308] border-2 border-white rounded-full flex items-center justify-center text-black shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)]"
                    >
                        ...
                        return (
                            <Link key={item.href} href={item.href} onClick={() => setIsOpen(false)}>
                                ...
                                >
                                    ...
                                    <span className="font-bold text-sm uppercase tracking-wide relative z-10">{item.title}</span>
                                </motion.div>
                            </Link>
                        );
                    })}
                ...
                <div className="p-6 border-t border-gray-800">
                    <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        className="flex items-center gap-3 w-full px-4 py-3 text-red-400 rounded-xl transition-colors font-bold text-sm border border-transparent hover:border-red-900 hover:text-red-300"
                    >
                        <motion.span
