...
const events = [
  {
    ...
    image: "/Images/Student hackathon.jpg", 
    description: "A hands-on session learning the basics of Arduino and microcontroller programming.",
    link: "/events/embedded-systems-workshop",
    color: "bg-[#22C55E]", // Green Badge
    cardColor: "bg-white", // White Card
  },
  {
    id: 2,
    ...
    description: "Meet recruiters from top engineering firms and tech companies. Bring your CV!",
    link: "/events/career-fair",
    color: "bg-[#EAB308]", // Yellow Badge
    cardColor: "bg-white", // White Card
  },
  {
    id: 3,
    ...
    description: "Watch the top 5 teams battle it out with their autonomous robots.",
    link: "/events/robotics-finals",
    color: "bg-white text-black", // White Badge (since card is black)
    cardColor: "bg-black text-white", // Black Card
  },
];
...
const EventCard = ({ event }: { event: any }) => {
  ...
  return (
    <motion.div 
      ...
      className={`group flex flex-col h-full ${event.cardColor} border-3 border-black rounded-2xl overflow-hidden transition-all duration-300 ${
        isBlackCard 
    ...
    >
      ...
      <div className="flex flex-col flex-grow p-6">
         ...
         <div className={`mt-auto flex items-center gap-3 pt-4 border-t-2 ${isBlackCard ? 'border-gray-800' : 'border-gray-100'}`}>
            ...
            <Link 
              href={event.link} 
              className={`flex-1 text-center py-3 rounded-lg font-bold text-sm border-2 transition-all ${
                isBlackCard
            ...
            <button 
              onClick={handleShare}
              className={`w-12 h-12 flex items-center justify-center rounded-lg border-2 transition-all relative overflow-hidden ${
                isBlackCard
