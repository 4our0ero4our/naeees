        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden py-6 space-y-2 custom-scrollbar">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <div
                  className={`flex items-center gap-4 px-3 py-3 rounded-xl transition-all duration-200 border-2 whitespace-nowrap overflow-hidden ${
                    isActive 
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    <div className="relative h-screen w-full bg-[#F8F9FA] overflow-hidden">
      ...
      <div 
        className="h-screen w-full relative transition-all duration-300"
        style={{
      ...
      >
        ...
        <header 
          className="fixed top-0 h-20 bg-white border-b-2 border-gray-200 flex items-center justify-between px-6 z-20 transition-all duration-300 right-0"
          style={{
            left: isDesktop ? (isCollapsed ? '5rem' : '18rem') : '0'
          }}
        >
           <div className="flex items-center gap-4">
               <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 text-black hover:bg-gray-100 rounded-lg border-2 border-transparent hover:border-black transition-all">
                 <FaBars className="text-xl" />
               </button>
               <h2 className="hidden sm:block font-heading font-black text-2xl text-black uppercase tracking-tighter">
                  Overview
               </h2>
           </div>
           ...
           <div className="flex items-center gap-4">
              ...
              <div className="w-10 h-10 bg-black border-2 border-black rounded-full overflow-hidden cursor-pointer hover:scale-105 transition-transform">
                 <div className="w-full h-full bg-gradient-to-br from-[#22C55E] to-[#EAB308]"></div>
              </div>
           </div>
        </header>
        ...
        <main className="h-screen overflow-y-auto overflow-x-hidden p-4 sm:p-6 lg:p-8 bg-[#F8F9FA] pt-24">
